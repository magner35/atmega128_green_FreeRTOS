...
// modified for ATmega128 by magner
...
void xSerialPrintf(const char *format, ...)
{
	va_list arg;
	va_start(arg, format);
	while (xSerialPort.serialWorkBufferInUse == ENGAGED)
		taskYIELD();
...
void xSerialxPrintf(xComPortHandlePtr pxPort, const char *format, ...)
{
	va_list arg;
	va_start(arg, format);
	while (pxPort->serialWorkBufferInUse == ENGAGED)
		taskYIELD();
...
xComPortHandle xSerialPortInitMinimal(eCOMPort ePort, uint32_t ulWantedBaud, uint16_t uxTxQueueLength, uint16_t uxRxQueueLength)
{
	...
	case USART0:
		...
		baud = (uint16_t)((configCPU_CLOCK_HZ + ulWantedBaud * 4UL) / (ulWantedBaud * 8UL) - 1); // for 2x mode, using 16 bit avr-gcc capability.
	...
	case USART1:
		baud = (uint16_t)((configCPU_CLOCK_HZ + ulWantedBaud * 4UL) / (ulWantedBaud * 8UL) - 1);
...
#if (0) // define this routines in yaMBSiavr.c
ISR(USART1_RX_vect)
{
	uint8_t cChar;
	/* Get status and data from buffer */
