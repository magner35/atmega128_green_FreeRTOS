...
#define LCD_FUNCTION_4BIT_2LINES_RUS 0x2A /* 4-bit interface, dual line, 5x7 dots, russian font table for OLED version*/
...
// uint8_t *LCDWorkBuffer; // create a working buffer pointer, to later be pvPortMalloc() on the heap.
...
extern void lcd_Printf_P(PGM_P format, ...);

void lcd_PutBar(uint8_t, uint8_t);
void lcd_createChar(uint8_t chr, const __flash uint8_t *p);
