/** \file avr_helper.h
 *  \brief Вспомогательные макросы для работы с \b AVR
 *
 *  \details
 *  Макросы для часто требующихся действий при разработке ПО для микроконтроллеров емейства \b AVR.
 *
 *  \author \b ARV
 */
...
/// Конкатенация: вспомогательный макрос слияния символов
#define CONCAT(y, x) _CONCAT_(y, x)

/// обращение к порту направления ввода-вывода по символу
#define DDR(x) CONCAT(DDR, x)
/// обращение к порту данных ввода-вывода по символу
#define PORT(x) CONCAT(PORT, x)
/// обращение к порту ввода по символу
#define PIN(x) CONCAT(PIN, x)

/// макрос для определения функции-инициализатора (вызывается автоматически в секции \b .inix)
#define INIT(x) static void __attribute__((naked, used, section(".init" #x))) CONCAT(_init_, __COUNTER__)(void)
/// макрос для определения завершающей функции (вызывается автоматически в секции \b .fini)
...
/// запрет начальной инициализации переменной
